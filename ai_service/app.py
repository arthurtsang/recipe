        ...
        try:
            ...
            try:
                ...
                if response_text.strip().startswith('{') and not response_text.strip().endswith('}'):
                    print(f"[import-recipe] Attempting to complete incomplete JSON...")
                    completion_prompt = (
                        "Complete this incomplete JSON to make it valid. "
                        "Add any missing closing braces and ensure all fields are properly closed:\n\n"
                        f"{response_text}\n\n"
                        "Complete JSON:"
                    )
                    ...
                    with torch.no_grad():
                        completion_outputs = model.generate(**completion_inputs, max_new_tokens=100, pad_token_id=tokenizer.eos_token_id)
                    ...
                    print(f"[import-recipe] Completion response: {completion_response}")
                    
                    try:
                        # Try to parse the completed JSON
                        data = json.loads(completion_response)
                        print(f"[import-recipe] Successfully parsed completed JSON: {data}")
                    except json.JSONDecodeError:
                        print(f"[import-recipe] Still invalid JSON after completion attempt")
                        data = {}
                else:
                    ...
                    if match:
                        ...
                        try:
                            ...
                            print(f"[import-recipe] Parsed data with regex: {data}")
                        except json.JSONDecodeError:
                            print(f"[import-recipe] Regex match is still invalid JSON")
                            data = {}
                    else:
                        print(f"[import-recipe] No JSON found in response")
                        data = {}
        except Exception as e:
